...
class ScanMatcher:
    ...
    def _build_local_grid(self, scan_points: np.ndarray) -> np.ndarray:
        ...
        # 2. Grid centre offset
        offset = self.local_grid_size // 2

        # 3. Convert (x, y) → (col, row)
    ...
    def _score_alignment(self, grid: np.ndarray, scan_points: np.ndarray,
        ...
        offset = self.local_grid_size // 2
        cols = np.floor(px_t / self.local_grid_resolution).astype(int) + offset
        rows = np.floor(py_t / self.local_grid_resolution).astype(int) + offset
